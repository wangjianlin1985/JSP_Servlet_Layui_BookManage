...
<script type="text/javascript">
    layui.use('table',function () {
        ...
        table.render({
            elem:'#bookCase-list',
            url:'<%=request.getContextPath()%>/admin/bookAdmin.do?method=bookCase&type=find',//数据接口
            page:true,//开启分页
            cols:[[//表头
        ...
        table.on('tool(bookCase-list)',function (obj) {
            var data = obj.data;
            if (obj.event == 'edit'){
                window.location.href = "<%=request.getContextPath()%>/admin/bookAdmin.do?method=bookCase&type=preEdit&bookCaseId=" + data.id;
            } else if (obj.event == 'del'){
               layer.confirm("确定删除该条记录吗？",function (index) {
                   window.location.href = "<%=request.getContextPath()%>/admin/bookAdmin.do?method=bookCase&type=delete&bookCaseId=" + data.id;
                    layer.close(index);
               })
            }
        })
