...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/css/reader.css">
    <script type="text/javascript" src="<%=request.getContextPath()%>/layui/layui.js"></script>
    <style>
        body {
            background: url("<%=request.getContextPath()%>/images/0.jpg");
...
<body>
    <div class="layui-container-box" style="background: url(../images/bg.jpg);background-size: cover">
        <div style="margin-left: 500px; width: 620px;">
            <a href="<%=request.getContextPath()%>/index.jsp">首页</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="<%=request.getContextPath()%>/admin/bookAdmin.do?method=exportDataInfo&type=reader">导出数据</a>&nbsp;&nbsp;|&nbsp;&nbsp;欢迎回来！<a href="<%=request.getContextPath()%>/reader/reader.jsp">${sessionScope.reader.name}</a><a href="<%=request.getContextPath()%>/account.do?method=logout"><button class="layui-btn layui-btn-warm layui-btn-radius">注销</button></a>
        </div>
...
<script>
    layui.use('table', function () {
        ...
        table.render({
            elem: '#borrow',
            url: '<%=request.getContextPath()%>/reader/reader.do',
            // cellMinWidth: 80,//全局定义常规单元格的最小宽度，layui 2.2.1 新增属性
            cols: [[
                {field: 'id',  title: 'ID', sort: true},
                {field: 'book',  title: '图书',templet:function (d) {return d.book.name }},
                {field: '<%=request.getContextPath()%>/reader',  title: '读者',templet:function (d) { return d.reader.username }},
        ...
        table.on('tool(borrow)',function (obj) {
            var data = obj.data;
            if(obj.event === 'return'){
                window.location.href="<%=request.getContextPath()%>/reader/reader.do?method=return&borrowId="+data.id;
            }
        })
